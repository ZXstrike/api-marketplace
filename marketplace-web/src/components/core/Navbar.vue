<template>
  <header
    class="bg-white/80 dark:bg-gray-900/80 backdrop-blur-lg sticky top-0 z-50 border-b border-gray-200 dark:border-gray-800 justify-items-center">
    <nav class="container mx-auto px-6 py-4 flex justify-between items-center">
      ...
      <div v-if="!authStore.isLoggedIn" class=" flex items-center gap-6">
        ...
        <a href="/#pricing"
          class="text-gray-600 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 font-medium">Pricing</a>
        <a href="/#features"
          class="text-gray-600 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 font-medium">Features</a>
        <router-link to="/docs"
      ...
      <div v-if="!authStore.isLoggedIn" class=" flex items-center gap-4">
        <router-link to="/login"
          class="text-gray-600 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 font-semibold">Log
          In</router-link>
        <router-link to="/register"
          class="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-5 py-2.5 rounded-lg shadow-md">Sign
          Up</router-link>
      </div>
      ...
      <div v-if="authStore.isLoggedIn" class=" lg:flex items-center gap-4">
        ...
        <div class="relative">
          <button @click="isProfileOpen = !isProfileOpen" class="flex items-center gap-3">
            ...
            <i data-feather="chevron-down" class="w-4 h-4 text-gray-500 transition-transform"
              :class="{ 'rotate-180': isProfileOpen }"></i>
          </button>
          <!-- Profile Dropdown Menu -->
          <div v-if="isProfileOpen" @click="isProfileOpen = false"
            class="absolute right-0 mt-2 w-48 bg-white dark:bg-gray-800 rounded-md shadow-xl z-50 border dark:border-gray-700 py-1">
            <router-link to="/dashboard"
              class="block px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700">Dashboard</router-link>
            <router-link to="/settings"
              class="block px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700">Settings</router-link>
            <div class="border-t border-gray-200 dark:border-gray-700 my-1"></div>
            <a href="#" @click.prevent="handleLogout"
              class="block w-full text-left px-4 py-2 text-sm text-red-600 dark:text-red-400 hover:bg-gray-100 dark:hover:bg-gray-700">Log
              Out</a>
          </div>
        </div>
      </div>
